...
sequenceDiagram
    ...
    Auth-->>FE: 200 OK + auth_token cookie + JWT in body
    
    FE->>Focus: POST /auth/session (Authorization: Bearer JWT)
    Focus->>Auth: gRPC Authenticate(JWT)
    Auth-->>Focus: user_id
    Focus->>DB: Create auth_session (sid, user_id, token)
    Focus-->>FE: 204 No Content + Set-Cookie: focus_sid, focus_csrf<br/>(SameSite=None; Secure)
    
    Note over FE,Focus: Subsequent API requests
    
    FE->>Focus: GET /sessions/running<br/>(Cookie: focus_sid, focus_csrf)
    Focus->>Focus: SessionAuthFilter validates<br/>focus_sid cookie + CSRF token
    Focus->>DB: Lookup session by sid
    DB-->>Focus: user_id
    Focus->>Focus: Execute use case
    Focus-->>FE: 200 OK + session data
```
...
#### Workloads
- **Deployment**: Focus service (2 replicas, HPA enabled)
- **StatefulSet**: PostgreSQL (single instance with persistent storage)

#### Networking
- **Service**: ClusterIP service exposing focus app
- **Ingress**: AWS ALB ingress with SSL/TLS (ACM certificate)
  - HTTPS redirect enabled
  - CORS headers configured

#### Configuration
- **ConfigMap**: Application configuration (Quarkus properties)
  - Database connection, gRPC client config, CORS origins
- **Secret**: Sensitive data (DB credentials)
- **ServiceAccount**: IRSA for ECR image pull
...
#### Storage
- **PVC**: EBS-backed persistent volume for PostgreSQL data

### Environment Configuration

**Docker Compose** (`compose.yml`): Local development
**Kubernetes** (Helm): Production deployment on AWS EKS

Configuration priority:
1. Kubernetes ConfigMap/Secrets (production)
2. `application.yaml` (defaults)
3. `application-docker.yaml` (docker-compose override)
...
- **Authentication**: gRPC client to Auth service
- **Security**: Cookie-based sessions with CSRF protection
- **Deployment**: Kubernetes (EKS) via Helm charts
